...
/* wwpdb change log
* 2013-06-07, RPSala: tweaked _fnShowTranspose() just to use more specific/qualified DOM locators for 
* 						'.dataTables_scrollHeadInner' and '.dataTables_scrollFootInner' elements
*/
...
Transpose.prototype = {
  ...
  "_fnShowTranspose": function ()
  {
    ...
    $('#'+sTableId+'_wrapper .dataTables_scrollHeadInner').hide();  // hide scroll head (when sScrollX is used)
    $('#'+sTableId+'_wrapper .dataTables_scrollFootInner').hide();  // hide scroll foot (when sScrollX is used)
    ...
    for (i = 1; i <= oSettings.aoColumns.length; i++) {  // nodes are 1-indexed in :nth-child
        // start a new row
        var $nTr = $('<tr>');
		$nTr.addClass(oSettings.asStripeClasses[(i+1) % oSettings.asStripeClasses.length]);

        // when sScrollX is used, the header is placed into a separate div. get the header and place into column 0
        if ($('#'+sTableId+'_wrapper .dataTables_scrollHeadInner').length) {
          $('#'+sTableId+'_wrapper .dataTables_scrollHeadInner thead th:nth-child('+i+')').each( function() {
